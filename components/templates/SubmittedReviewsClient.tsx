...
import ReviewViewer from '@/components/organisms/ReviewViewer';
...
function SubmittedReviewsClient() {
  ...
  const {
    ...
    queryFn: () => reviewApi.getMyWrittenReviews({ page, pageSize }),
    ...
    return (
      <div className="bg-background-100 flex items-center justify-center h-screen">
        <div className="text-red-600">
          🥺 작성한 리뷰 목록을 불러오는 데 실패했습니다: {error.message}
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-BackGround-100 min-h-full">
          ...
          <div className="flex flex-wrap w-full justify-center lg:justify-start pt-2">
            {reviews.length === 0 && !isFetching ? (
              ...
              reviews.map((review) => (
                <div
                  key={review.id}
                  className="pt-8 lg:pt-10 lg:pr-3 w-full lg:w-1/2 flex justify-center cursor-pointer" // 클릭 가능한 UI 표현
      ...
      {isModalOpen && selectedReview && (
        <GeneralModal onClose={handleCloseModal} isOpen={isModalOpen}>
          <ReviewViewer onClose={handleCloseModal} review={selectedReview} />
        </GeneralModal>
      )}
    </div>
  );
