...
//     queryFn: () => 
//       reviewsApi.getMyWrittenReviews({ page, pageSize }), 
...
function SubmittedReviewsClient() {
  ...
  const {
    data: reviewsData,
    isLoading,
    isFetching, // isFetching 상태를 계속 사용
    isError,
    error,
  } = useQuery<{ list: Review[]; totalCount: number }, Error>({
    queryKey: ['myWrittenReviews', { page, pageSize }],
  ...
  // useMemo를 사용하여 reviews 목록 메모이제이션
  const reviews = useMemo(() => reviewsData?.list || [], [reviewsData?.list]);
  const totalCount = reviewsData?.totalCount || 0;
  const totalPages = totalCount > 0 ? Math.ceil(totalCount / pageSize) : 0; // 이미 올바르게 계산되고 있었습니다.

  useEffect(() => {
    if (reviewsData) {
      console.log('작성한 리뷰 데이터:', reviewsData);
      console.log('현재 페이지:', page, '페이지 크기:', pageSize, '총 아이템:', totalCount, '총 페이지:', totalPages);
    }
  }, [reviewsData, page, pageSize, totalCount, totalPages]);
  ...
  return (
    <div className="bg-BackGround-100 min-h-full">
      <div className="flex items-center justify-center">
        <div className="flex flex-col justify-between items-center w-[327px] md:w-[600px] lg:w-[1400px]">
          {/* 데이터 패칭 중일 때 (페이지 이동 등) 로딩 메시지 */}
          {isFetching && !isLoading && ( // isLoading이 아닐 때만 isFetching 로더를 표시 (중복 방지)
            <div className="text-center text-gray-500 my-4">데이터 업데이트 중...</div>
          )}
          <div className="flex flex-wrap w-full justify-center lg:justify-start pt-2">
            {reviews.length === 0 && !isFetching ? ( // !isFetching 조건을 추가하여, 데이터가 없는 것과 로딩 중인 것을 구분
              <div className="w-full flex justify-center items-center mt-[50px] min-h-[370px]">
                <EmptyListMessage
                  message="작성한 리뷰가 없습니다."
                  isUsingButton={true}
              ...
              reviews.map((review) => (
                <div key={review.id} className="pt-8 lg:pt-10 lg:pr-3 w-full lg:w-1/2 flex justify-center">
                  <WorkerCardInCompletedReview
                    ...
                    price={review.price}
                    content={review.content}
                    createdAt={new Date(review.createdAt)} // DB에서 ISO 문자열로 온다고 가정
                    rating={review.star}
                  />
                </div>
              ))
