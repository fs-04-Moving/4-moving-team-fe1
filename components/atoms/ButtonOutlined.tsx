 ...
 * @returns
 */
function ButtonOutlined({ intent = 'default', children, className, ...props }: Props) {
  ...
  const activeClassName = intent === 'active' && '!bg-Primay-Blue-50 hover:!bg-Primay-Blue-100';
  const doneClassName = intent === 'done' && '!border-GrayScale-100 !text-GrayScale-300';
  const disabledClassName = disabled && '!cursor-default opacity-50';

  return (
    <button
      className={clsx(baseClassName, activeClassName, doneClassName, disabledClassName, className)}
      {...props}
    >
      {children}
    </button>
