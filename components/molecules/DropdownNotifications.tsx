...
const DropdownNotification = forwardRef<HTMLDivElement, DropdownNotificationProps>(
            ...
            notifications.map((notification) => (
              ...
              >
                <div className="flex justify-between items-start mb-2">
                  <p
                    className={clsx('text-Black-400 font-medium text-sm lg:text-base', {
                ...
                <p className="text-sm text-GrayScale-400">{timeAgo(notification.createdAt)}</p>
              </div>
            ))
          ) : (
            <div className="py-8 text-center text-GrayScale-400">새로운 알림이 없습니다.</div>
          )}
        </div>
      </div>
    );
